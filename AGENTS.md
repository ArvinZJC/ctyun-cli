...
- `internal/openapi` owns the dev-only OpenAPI harvest/review pipeline, and `tools/openapi` is only a repository-local maintenance entrypoint. The pipeline works from normalized catalogs under `openapi/products/<name>/`, keeps generated drafts separate from promoted plugin metadata, and advances `baseline.json` only after a reviewed or curated promotion. Do not expose it as a public `ctyun` command or package OpenAPI working artifacts into core/plugin release outputs.
...
- Commit subjects must use one of these prefixes only: `feat:`, `fix:`, `docs:`, `style:`, `build:`, `refactor:`, `revert:`, `test:`, `perf:`, `ci:`, or `chore:`. Keep the subject in concise non-sentence style without trailing punctuation.
