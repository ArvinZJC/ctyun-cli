...
| Media Storage | `media-storage` | `media-storage` | [![GitHub Tag](https://img.shields.io/github/v/tag/ArvinZJC/ctyun-cli?filter=releases%2Fplugins%2Fmedia-storage%2F*&label=release)](../../releases) | `beta` | `generated` | 73 | 73 |
...
The `media-storage` and `classic-object-storage` plugins cover 73 of 74 captured media APIs and 106 of 106 captured Classic Type I APIs. Both use CTyun OpenAPI gateways and EOP signing and require core `>=0.5.0 <1.0.0`. The published media endpoint covers Tibet resource pool 1. Only POST upload remains excluded because it needs separate policy signing. Example corrections, corroborating evidence, and the explicitly synthetic download fixture are recorded in the [media inventory](openapi-catalogs/media-storage/coverage.json) and [classic inventory](openapi-catalogs/classic-object-storage/coverage.json).
