...
| Virtual Private Cloud                     | `vpc`                    | `vpc`                    | [![GitHub Tag](https://img.shields.io/github/v/tag/ArvinZJC/ctyun-cli?filter=releases%2Fplugins%2Fvpc%2F*&label=release)](../../releases)                    | `beta`   | `generated` |      193 |        193 |
...
- Redis: `redis` is based on the API catalog officially labeled “2023-10-18 II类资源池” (`vid=270`). This Redis catalog label does not mean the joint-operation pools (second-class nodes) excluded by this project. Historical APIs are under `ctyun redis legacy`; it requires core `>=0.5.0 <1.0.0`. All commands include offline response fixtures; the [coverage inventory](openapi-catalogs/redis/coverage.json) records source corrections and remaining waiter gaps.
- MongoDB: older APIs are under `ctyun mongodb v1`; the plugin requires core `>=0.5.0 <1.0.0`. Three export APIs remain unavailable because their binary response contracts are unclear. All supported commands include offline response fixtures; see the [coverage inventory](openapi-catalogs/mongodb/coverage.json).
