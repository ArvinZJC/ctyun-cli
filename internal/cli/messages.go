...
var messageCatalog = map[string]map[string]string{
	...
	"error.invalid_option_value_type":            {"en-US": "invalid value %q for option %q: expected %s", "en-GB": "invalid value %q for option %q: expected %s", "zh-CN": "值 %q 对选项 %q 无效：应为 %s"},
	...
	"error.recommendation_applicability":         {"en-US": "recommendation applicability must not be blank", "en-GB": "recommendation applicability must not be blank", "zh-CN": "推荐适用范围不能为空"},
	...
	"error.waiter_ambiguous_resource":            {"en-US": "waiter response contains multiple matches for resource %q", "en-GB": "waiter response contains multiple matches for resource %q", "zh-CN": "等待器响应包含资源 %q 的多个匹配结果"},
	"error.waiter_expected_collection":           {"en-US": "waiter path %q must contain an array", "en-GB": "waiter path %q must contain an array", "zh-CN": "等待器路径 %q 必须包含数组"},
	"error.waiter_expected_scalar":               {"en-US": "waiter path %q must contain a scalar value", "en-GB": "waiter path %q must contain a scalar value", "zh-CN": "等待器路径 %q 必须包含标量值"},
	"error.waiter_empty_example":                 {"en-US": "waiter requires a nonempty collection example", "en-GB": "waiter requires a nonempty collection example", "zh-CN": "等待器需要非空集合示例"},
	"error.waiter_invalid_selector":              {"en-US": "waiter %q requires a collection path, identity key and command bindings", "en-GB": "waiter %q requires a collection path, identity key and command bindings", "zh-CN": "等待器 %q 需要集合路径、标识键和命令绑定"},
	"error.waiter_requires_input":                {"en-US": "waiter requires input %q to select one resource", "en-GB": "waiter requires input %q to select one resource", "zh-CN": "等待器需要输入 %q 以选择单个资源"},
	"error.waiter_not_applicable":                {"en-US": "waiter %q cannot poll command %q", "en-GB": "waiter %q cannot poll command %q", "zh-CN": "等待器 %q 不适用于命令 %q"},
	"error.waiter_invalid_conditions":            {"en-US": "waiter %q requires a response path and nonempty, non-overlapping terminal values", "en-GB": "waiter %q requires a response path and nonempty, non-overlapping terminal values", "zh-CN": "等待器 %q 需要响应路径以及非空且互不重叠的终止状态值"},
	"error.waiter_invalid_binding":               {"en-US": "waiter %q references missing or unsafe command %q", "en-GB": "waiter %q references missing or unsafe command %q", "zh-CN": "等待器 %q 引用了不存在或不适合轮询的命令 %q"},
	...
	"error.expected_single_json_value":           {"en-US": "expected one JSON value", "en-GB": "expected one JSON value", "zh-CN": "需要单个 JSON 值"},
	"error.expected_json_response_object":        {"en-US": "expected a JSON response object", "en-GB": "expected a JSON response object", "zh-CN": "需要 JSON 响应对象"},
	...
	"notice.api_support":                         {"en-US": "If this looks like a ctyun CLI issue, open https://github.com/ArvinZJC/ctyun-cli/issues or https://gitee.com/ArvinZJC/ctyun-cli/issues. If this looks like a CTyun service issue, submit a CTyun work order.", "en-GB": "If this looks like a ctyun CLI issue, open https://github.com/ArvinZJC/ctyun-cli/issues or https://gitee.com/ArvinZJC/ctyun-cli/issues. If this looks like a CTyun service issue, submit a CTyun work order.", "zh-CN": "如果你认为这是 ctyun CLI 问题，请在 https://github.com/ArvinZJC/ctyun-cli/issues 提交 GitHub Issue，或在 https://gitee.com/ArvinZJC/ctyun-cli/issues 提交 Gitee Issue；如果你认为这是天翼云服务问题，请提交天翼云工单。"},
	...
	"error.command_parameter_invalid_default":    {"en-US": "option %s has invalid default %q for value type %s", "en-GB": "option %s has invalid default %q for value type %s", "zh-CN": "选项 %s 的默认值 %q 对值类型 %s 无效"},
	"error.command_parameter_default_disallowed": {"en-US": "command %s parameter %s default %q must be one of %s", "en-GB": "command %s parameter %s default %q must be one of %s", "zh-CN": "命令 %s 的参数 %s 默认值 %q 必须是以下值之一：%s"},
	"error.command_parameter_value_type":         {"en-US": "command %s parameter %s has unsupported value type %s", "en-GB": "command %s parameter %s has unsupported value type %s", "zh-CN": "命令 %s 的参数 %s 使用了不支持的值类型 %s"},
	...
	"error.command_conditional_mixed_modes":      {"en-US": "command %q mixes condition modes", "en-GB": "command %q mixes condition modes", "zh-CN": "命令 %q 混用了条件模式"},
	...
	"error.missing_unconditional_flag":           {"en-US": "requires --%s", "en-GB": "requires --%s", "zh-CN": "需要 --%s"},
	"error.missing_unconditional_any":            {"en-US": "requires one of %s", "en-GB": "requires one of %s", "zh-CN": "需要以下选项之一：%s"},
	"error.missing_composite_flag":               {"en-US": "requires --%s when %s", "en-GB": "requires --%s when %s", "zh-CN": "当 %[2]s 时需要 --%[1]s"},
	"error.missing_composite_any":                {"en-US": "requires one of %s when %s", "en-GB": "requires one of %s when %s", "zh-CN": "当 %[2]s 时需要以下选项之一：%[1]s"},
	"error.response_media_type":                  {"en-US": "unexpected HTTP response Content-Type %q; expected %q", "en-GB": "unexpected HTTP response Content-Type %q; expected %q", "zh-CN": "HTTP 响应的 Content-Type %q 不符合预期，需要 %q"},
