...
func TestNativeContractsRejectAmbiguousRouting(t *testing.T) {
	...
	valid := Native{Service: "s3", Addressing: "path", Bucket: "$arg.bucket", Object: "$param.object", Subresources: []string{"acl"}, V2QueryKeys: []string{"acl"}}
	if err := ValidateNative(&valid, "/", response); err != nil {
		t.Fatal(err)
	}
	for _, change := range []func(*Native){func(n *Native) { n.Service = "bad" }, func(n *Native) { n.Addressing = "bad" }, func(n *Native) { n.Bucket = "" }, func(n *Native) { n.Bucket = "literal" }, func(n *Native) { n.Subresources = []string{"a=b"} }, func(n *Native) { n.V2QueryKeys = []string{"acl", "acl"} }} {
		n := valid
		change(&n)
