...
const sensitiveField = `(?:signature|access[-_]?key(?:[-_]?id)?|awsaccesskeyid|secret[-_]?access[-_]?key|secret[-_]?key|session[-_]?token|security[-_]?token|authorization|password|base32stringseed|qrcodepng)`
