...
func TestReleaseToolWritesSignedIndexAndArchive(t *testing.T) {
	...
	if rel.Version != "0.5.0" {
		t.Fatalf("version = %s, want 0.5.0", rel.Version)
	}
...
func TestReleaseToolMergesExistingIndexes(t *testing.T) {
	...
	if rel, _, ok := coreIndex.FindLatest("stable", "darwin", "arm64"); !ok || rel.Version != "0.5.0" {
...
func TestReleaseToolWritesWindowsArtifactAndPropagatesBuildError(t *testing.T) {
	...
	defer restore()
	outDir := t.TempDir()
	err = run([]string{"--version", "0.5.0", "--out", outDir, "--platform", "windows/amd64"}, func(string) string {
		return base64.StdEncoding.EncodeToString(privateKey)
	}, &bytes.Buffer{})
	if err != nil {
		t.Fatalf("run windows release returned error: %v", err)
	}
	if _, err := os.Stat(filepath.Join(outDir, "ctyun_0.5.0_windows_amd64.tar.gz")); err != nil {
		t.Fatal(err)
	}
	...
	defer restoreFail()
	if err := run([]string{"--version", "0.5.0", "--out", t.TempDir(), "--platform", "linux/amd64"}, func(string) string {
		return base64.StdEncoding.EncodeToString(privateKey)
